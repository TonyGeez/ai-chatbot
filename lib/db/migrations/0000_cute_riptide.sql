DO $$ BEGIN
 CREATE TYPE "public"."provider_enum" AS ENUM('openrouter', 'deepinfra');
EXCEPTION
...
CREATE TABLE IF NOT EXISTS "ProviderConfig" (
	...
	CONSTRAINT "ProviderConfig_userId_provider_unique" UNIQUE("userId","provider")
);
--> statement-breakpoint
...
--> statement-breakpoint
DO $$ BEGIN
 ALTER TABLE "ProviderConfig" ADD CONSTRAINT "ProviderConfig_userId_User_id_fk" FOREIGN KEY ("userId") REFERENCES "public"."User"("id") ON DELETE no action ON UPDATE no action;
EXCEPTION
 WHEN duplicate_object THEN null;
END $$;
--> statement-breakpoint
